...
const roadGame=['racing','bike'].includes(type), stagedGame=roadGame||type==='aviation';
const stages=()=>type==='aviation'?flightLevels:raceLevels;
...
function fresh(){state=type==='snake'?createSnake():roadGame?createRace(type==='bike'?'bike':'car'):type==='aviation'?createFlight():createTank();if(type==='tank')spawnWave(state);acc=0;loadBest();updateStats();draw();}
function updateStats(){$('#score').textContent=state.score;$('#extra-stat').textContent=type==='tank'?`ARMOR ${state.armor} · WAVE ${state.wave}`:roadGame?`LEVEL ${state.level+1}/${raceLevels.length} · CONDITION ${state.armor} · ${Math.floor(state.distance)}/${raceLevels[state.level].target}m · ${type==='bike'?`THROTTLE ${Math.round(state.throttle*100)}%`:`PACE ${Math.round(state.speed/raceLevels[state.level].speed*100)}%`}`:type==='aviation'?`MISSION ${state.level+1}/${flightLevels.length} · HULL ${state.armor} · RINGS ${state.rings}/${flightLevels[state.level].rings}`:difficulty().toUpperCase();}
function start(advance=false){sound.unlock();sound.silence();if(advance&&stagedGame&&(type==='aviation'?nextFlightLevel(state):nextRaceLevel(state))){acc=0;updateStats();}else fresh();sound.cue('start');running=true;paused=false;clearInput();$('#overlay').hidden=true;$('#pause').disabled=false;$('#pause').textContent='Pause';if($('#difficulty'))$('#difficulty').disabled=true;$('#game-status').textContent='Game started';canvas.focus();}
...
function end(){running=false;sound.stopEngine();sound.cue(state.won?'win':'over');saveBest();clearInput();$('#overlay').hidden=false;$('#overlay-title').textContent=state.won?(roadGame?'Championship complete!':type==='aviation'?'All missions complete!':'Board complete!'):'One more round?';$('#overlay-copy').textContent=`You scored ${state.score}. Personal best: ${best}.`;$('#start').textContent='Play again ↗';$('#pause').disabled=true;if($('#difficulty'))$('#difficulty').disabled=false;$('#game-status').textContent=`Game over. Score ${state.score}.`;}
function completeStage(){running=false;clearInput();sound.stopEngine();sound.cue('level');saveBest();$('#overlay').hidden=false;$('#overlay-title').textContent=`Stage ${state.level+1} complete!`;$('#overlay-copy').textContent=`Next: ${stages()[state.level+1].name}. One condition point repaired.`;$('#start').textContent='Next stage →';$('#pause').disabled=true;$('#game-status').textContent=`Stage ${state.level+1} complete. Score ${state.score}.`; }
$('#start').onclick=()=>paused?togglePause():start(stagedGame&&state.stageComplete&&!state.won&&state.alive);$('#restart').onclick=()=>start();$('#pause').onclick=togglePause;
$('#difficulty')?.addEventListener('change',()=>{loadBest();updateStats();});
const shell=$('.game-shell');
function theater(enabled){shell.classList.toggle('theater',enabled);document.body.classList.toggle('game-expanded',enabled);$('#fullscreen').textContent=enabled?'Exit full screen ↙':'Full screen ↗';}
$('#fullscreen').onclick=async()=>{
 if(shell.classList.contains('theater')){theater(false);return;}
 if(document.fullscreenElement){await document.exitFullscreen();return;}
 try{if(!shell.requestFullscreen)throw Error();await shell.requestFullscreen();}catch{theater(true);}
};
document.addEventListener('fullscreenchange',()=>{$('#fullscreen').textContent=document.fullscreenElement?'Exit full screen ↙':'Full screen ↗';});
document.addEventListener('keydown',e=>{if(e.key==='Escape')theater(false);});
// Retain 720×540 logical physics while drawing sharply at the displayed size.
function resizeCanvas(){const rect=canvas.getBoundingClientRect(),dpr=Math.min(window.devicePixelRatio||1,2);const width=Math.round(rect.width*dpr),height=Math.round(rect.height*dpr);if(width&&height&&(canvas.width!==width||canvas.height!==height)){canvas.width=width;canvas.height=height;}ctx.setTransform(canvas.width/720,0,0,canvas.height/540,0,0);}
...
function frame(now){
 ...
 if(running&&!paused){
  ...
  if(type==='snake'){
   ...
   stepFlight(state,dt,{left:touchKeys.has('left')||keys.has('left'),right:touchKeys.has('right')||keys.has('right'),up:touchKeys.has('up')||keys.has('up'),down:touchKeys.has('down')||keys.has('down'),boost:touchFire||keys.has('fire')});
   sound.setEngine(state.speed);if(state.armor<before.armor)sound.cue('hit');else if(state.score>before.score&&!state.stageComplete)sound.cue('eat');
  }else{
  ...
  if(!state.alive||state.won)end();else if(stagedGame&&state.stageComplete)completeStage();
 }
 draw();requestAnimationFrame(frame);
}
window.addEventListener('pagehide',()=>sound.silence());
fresh();resizeCanvas();requestAnimationFrame(frame);
